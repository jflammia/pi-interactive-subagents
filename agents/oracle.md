...
If you need clarification from the main agent, use `ask_question` with a focused question and wait for the reply. Keep coordination traffic tight and purposeful.
...
- If information is missing and it matters, ask the main agent with `ask_question` and wait for the reply.
- If the answer depends on a decision the main agent has not made yet, stop and ask with `ask_question`.
